...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	if (isRichText) {
		...
		if (buttons == true) {
			document.writeln('<table class="rteBack" cellpadding=4 cellspacing=0 id="Buttons1_' + rte + '" width="' + tablewidth + '">');
			document.writeln('	<tr>');
			document.writeln('		<td><img src=images/blank.gif width=5 height=30></td><td>');
			...
			document.writeln('				<option value="Font" selected>[Font Type]</option>');
			document.writeln('				<option value="Arial, Helvetica, sans-serif">Arial</option>');
			document.writeln('				<option value="Arial Narrow, Arial, Helvetica, sans-serif">Arial Narrow</option>');
			...
			document.writeln('		  <input type=button value="Insert Merge Field" onclick="javascript:dlgField(\'' + rte + '\', \'field\', \'\')">&nbsp;&nbsp;');
			document.writeln('		</td>');
			document.writeln('	</tr>');
...
function dlgField(rte, command) {
  ...
  if (id)
		InsertTable = popUpWin(includesPath + 'insert_field.php?view_id=' + id, 'InsertField', 300, 350, 'scrollbars=1,');
	else
	  alert("You need to select a view first");
}
