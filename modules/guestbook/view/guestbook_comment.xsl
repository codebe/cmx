      ...
      <table width="100%" class="tableHandler" cellpadding="0" cellspacing="1">
        <tr>			
		  ...
		  <xsl:attribute name="id"><xsl:value-of select="//blockID" />_handler</xsl:attribute>
		   <em>Guestbook Comment <xsl:value-of select="//blockID" /></em> &nbsp;</td>
        ...
        <tr><xsl:attribute name="id"><xsl:value-of select="//blockID" />_section</xsl:attribute>
          <td colspan="2"><table width="100%" cellspacing="5" cellpadding="0">
              ...
              <tr>
                <td><strong><xsl:value-of select="name" /></strong>, <xsl:value-of select="email" /> at <em><xsl:value-of select="entry_date" /></em></td>				
              </tr>
			  <tr>
                <td><em><xsl:value-of select="comment" /></em><br /><br /></td>				
              </tr>
			  </xsl:for-each>
              <tr><td>Back to fill form, Go to<a>
			  <xsl:attribute name="href">#<xsl:value-of select="//blockID" /></xsl:attribute>
			  <xsl:attribute name="onClick">showGuestbookPage(<xsl:value-of select="//blockID" />)</xsl:attribute>
			   guestbook page</a></td>
              </tr>
            </table></td>
        </tr>
